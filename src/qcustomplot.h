...
protected:
	...
	void drawPolyline(QCPPainter* painter, const QVector<QPointF>& line) const;
...
*/
template <class DataType>
void QCPAbstractPlottable1D<DataType>::drawPolyline(QCPPainter* painter, const QVector<QPointF>& line) const
{
		...
		!painter->modes().testFlag(QCPPainter::pmNoCaching))
	{
		...
		const int lineDataSize = line.size();
		while (i < lineDataSize && (qIsNaN(line.at(i).y()) || qIsNaN(line.at(i).x()))) // make sure first point is not NaN
			++i;
		++i; // because drawing works in 1 point retrospect
		while (i < lineDataSize)
		{
			if (!qIsNaN(line.at(i).y()) && !qIsNaN(line.at(i).x())) // NaNs create a gap in the line
			{
				if (!lastIsNan)
	...
	else
	{
		...
		while (i < lineDataSize)
		{
			if (qIsNaN(line.at(i).y()) || qIsNaN(line.at(i).x()) || qIsInf(line.at(i).y())) // NaNs create a gap in the line. Also filter Infs which make drawPolyline block
			{
				painter->drawPolyline(line.constData() + segmentStart, i - segmentStart); // i, because we don't want to include the current NaN point
				segmentStart = i + 1;
			}
			++i;
		}
		// draw last segment:
		painter->drawPolyline(line.constData() + segmentStart, lineDataSize - segmentStart);
	}
}
...
protected:
	...
	virtual void getOptimizedLineData(QVector<QCPGraphData>* line, const QCPGraphDataContainer::const_iterator& begin, const QCPGraphDataContainer::const_iterator& end) const;
